...
//
// 既存の作品カードは、一覧では表紙画像(あれば)+作品名+「詳しく見る」だけを表示する。
// プロフィール的な詳細情報(出版社・掲載媒体・連載状況・単行本巻数等)は詳細ページのみに表示する。
function renderCard(item) {
  ...
  const coverHtml = item.image ? `<img class="card-cover" src="${escapeHtml(item.image)}" alt="">` : "";
  return `
    <article class="card">
      ${coverHtml}
      <div class="card-body">
